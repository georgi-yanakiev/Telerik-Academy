...
class TreeNode<K, V> where K : IComparable
{
    ...
    public void AddChild(TreeNode<K, V> childNode)
    {
        ...
        {
            if (rightChild != null)
                rightChild.AddChild(childNode);
            else
                rightChild = new TreeNode<K, V>(childNode.key, childNode.value);
        }
        else if (this.key.CompareTo(childNode.key) > 0)
        {
            if (leftChild != null)
                leftChild.AddChild(childNode);
            else
                leftChild = new TreeNode<K, V>(childNode.key, childNode.value);
        }
        else
